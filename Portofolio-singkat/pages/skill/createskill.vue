...
export default {
  ...
  methods: {
    handleFileChange(event) {
      ...
      if (selectedFile) {
        this.skill.image = selectedFile // Set the selected file as the 'image' property
      }
    },
    saveProduct() {
      ...
      formData.append('link', this.skill.link)
      ...
      axios
        .post('http://127.0.0.1:8000/api/skills', formData) // Use formData here, not FormData
        .then((res) => {
          console.log(res, 'res')
          alert(res.data.message)
          this.skill.name = ''
          this.skill.description = ''
          this.skill.image = null // Clear the image field
